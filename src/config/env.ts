/**
 * A source of environment variables (the Oxygen `env` binding).
 *
 * Unlike Node-based frameworks, Oxygen has no global `process.env`, so configuration is read from
 * the per-request `env` binding instead.
 *
 * @internal
 */
